...
//  - admin:    checked in the browser (there is no admin backend yet) — replace with a server call.
//  - lecturer: verified by the Flask API; this is the account seeded in backend/lecturer-api.
...
async function handleSignIn() {
  ...
  setBusy(true);
  try {
    if (currentRole === 'admin')    signInAdmin(email, password);
...
function signInAdmin(email, password) {
  ...
  if (email.toLowerCase() !== c.email || password !== c.password) {
    throw new Error('Invalid email or password. Please try again.');
  }
  SUSLSession.set('admin', { name: 'Administrator', email: c.email });
  finishSignIn('admin');
}
